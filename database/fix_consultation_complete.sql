-- 完整修复咨询系统 RLS 策略
-- 确保：1. role=user 可以创建队列 2. role=doctor 可以看到队列 3. doctor 可以接受并进入聊天室
-- 在 Supabase SQL Editor 中运行此脚本

-- ============================================
-- 步骤 1: 删除所有现有的 consultation_queue RLS 策略
-- ============================================
DO $$
DECLARE
  policy_record RECORD;
BEGIN
  FOR policy_record IN 
    SELECT policyname 
    FROM pg_policies 
    WHERE tablename = 'consultation_queue'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.consultation_queue', policy_record.policyname);
    RAISE NOTICE 'Dropped policy: %', policy_record.policyname;
  END LOOP;
END $$;

-- 确保 RLS 已启用
ALTER TABLE public.consultation_queue ENABLE ROW LEVEL SECURITY;

-- ============================================
-- 步骤 2: 创建 SELECT 策略（查看队列）
-- ============================================

-- 策略 1: 用户可以看到自己的队列
CREATE POLICY "Users can view own queue"
  ON public.consultation_queue
  FOR SELECT
  USING (auth.uid() = patient_id);

-- 策略 2: Admin 可以查看所有 waiting 队列
CREATE POLICY "Admins can view all waiting queues"
  ON public.consultation_queue
  FOR SELECT
  USING (
    status = 'waiting' AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'admin'
    )
  );

-- 策略 3: Admin 可以查看所有 accepted/in_chat 队列
CREATE POLICY "Admins can view matched queues"
  ON public.consultation_queue
  FOR SELECT
  USING (
    status IN ('accepted', 'in_chat') AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'admin'
    )
  );

-- 策略 4: role=doctor 可以查看所有 waiting 队列
CREATE POLICY "Doctors can view waiting queues"
  ON public.consultation_queue
  FOR SELECT
  USING (
    status = 'waiting' AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'doctor'
    )
  );

-- 策略 5: role=doctor 可以查看所有 accepted/in_chat 队列
CREATE POLICY "Doctors can view matched queues"
  ON public.consultation_queue
  FOR SELECT
  USING (
    status IN ('accepted', 'in_chat') AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'doctor'
    )
  );

-- 策略 6: 链接了 pharmacist account 的用户可以查看 waiting 队列（向后兼容）
CREATE POLICY "Linked pharmacists can view waiting queues"
  ON public.consultation_queue
  FOR SELECT
  USING (
    status = 'waiting' AND 
    EXISTS (
      SELECT 1 FROM public.doctors
      WHERE doctors.user_id = auth.uid()
    )
  );

-- ============================================
-- 步骤 3: 创建 INSERT 策略（创建队列）
-- ============================================

-- 策略 1: role=user 可以创建队列
CREATE POLICY "Users can create queue"
  ON public.consultation_queue
  FOR INSERT
  WITH CHECK (
    auth.uid() = patient_id AND
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'user'
    )
  );

-- 策略 2: 如果没有 role，但 auth.uid() = patient_id，也允许（向后兼容）
CREATE POLICY "Authenticated users can create own queue"
  ON public.consultation_queue
  FOR INSERT
  WITH CHECK (auth.uid() = patient_id);

-- ============================================
-- 步骤 4: 创建 UPDATE 策略（更新队列）
-- ============================================

-- 策略 1: 用户可以更新自己的队列（取消等）
CREATE POLICY "Users can update own queue"
  ON public.consultation_queue
  FOR UPDATE
  USING (auth.uid() = patient_id)
  WITH CHECK (auth.uid() = patient_id);

-- 策略 2: role=doctor 可以接受 waiting 队列
CREATE POLICY "Doctors can accept queues"
  ON public.consultation_queue
  FOR UPDATE
  USING (
    status = 'waiting' AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'doctor'
    )
  )
  WITH CHECK (
    consultation_queue.status IN ('accepted', 'in_chat')
  );

-- 策略 3: role=doctor 可以更新已接受的队列
CREATE POLICY "Doctors can update accepted queues"
  ON public.consultation_queue
  FOR UPDATE
  USING (
    status IN ('accepted', 'in_chat') AND 
    EXISTS (
      SELECT 1 FROM public.user_profiles
      WHERE user_profiles.id = auth.uid()
      AND user_profiles.role = 'doctor'
    )
  )
  WITH CHECK (
    consultation_queue.status IN ('accepted', 'in_chat', 'completed')
  );

-- 策略 4: 链接了 pharmacist account 的用户可以接受队列（向后兼容）
CREATE POLICY "Linked pharmacists can accept queues"
  ON public.consultation_queue
  FOR UPDATE
  USING (
    status = 'waiting' AND 
    EXISTS (
      SELECT 1 FROM public.doctors
      WHERE doctors.user_id = auth.uid()
    )
  )
  WITH CHECK (
    consultation_queue.status IN ('accepted', 'in_chat')
  );

-- 策略 5: 链接了 pharmacist account 的用户可以更新已接受的队列（向后兼容）
CREATE POLICY "Linked pharmacists can update accepted queues"
  ON public.consultation_queue
  FOR UPDATE
  USING (
    status IN ('accepted', 'in_chat') AND 
    EXISTS (
      SELECT 1 FROM public.doctors
      WHERE doctors.id = consultation_queue.pharmacist_id
      AND doctors.user_id = auth.uid()
    )
  )
  WITH CHECK (
    consultation_queue.status IN ('accepted', 'in_chat', 'completed')
  );

-- ============================================
-- 步骤 5: 验证策略已创建
-- ============================================
SELECT 
  '✅ Verification: All Policies Created' AS status,
  policyname,
  cmd AS command
FROM pg_policies
WHERE tablename = 'consultation_queue'
ORDER BY 
  CASE cmd
    WHEN 'SELECT' THEN 1
    WHEN 'INSERT' THEN 2
    WHEN 'UPDATE' THEN 3
    ELSE 4
  END,
  policyname;

-- ============================================
-- 完成！
-- ============================================
-- 现在：
-- 1. role=user 可以创建队列（INSERT）
-- 2. role=doctor 可以看到所有 waiting 队列（SELECT）
-- 3. role=doctor 可以接受队列并进入聊天室（UPDATE）

