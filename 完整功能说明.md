...
2. **两个队列组件并存**
   - `ConsultationWaiting.jsx` - 旧版本（有自动匹配）
   - `ConsultationQueue.jsx` - 新版本（等待管理员接受）
   - 当前 `App.jsx` 使用 `ConsultationQueue`

### 🔄 代码状态
- 已恢复到提交 `47d10ac` 的版本（使用 `ConsultationWaiting`）
- 但 `App.jsx` 仍在使用 `ConsultationQueue`
- 需要统一使用一个组件
